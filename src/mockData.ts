...
export function buildHistory(tasks: Task[]): DayHistory[] {
    ...
    for (let i = 27; i >= 0; i--) {
        ...
        days.push({ date: dateStr, label, dayNum: d.getDate(), completed, total, rate: total > 0 ? completed / total : 0 });
    }
    return days;
}
...
export function getCategoryStats(tasks: Task[], history: DayHistory[]): CategoryStat[] {
    ...
    const overallRate = history.length > 0
        ? history.reduce((sum, d) => sum + d.rate, 0) / history.length
        : 0.65;
